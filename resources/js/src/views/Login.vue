        ...
        <v-col lg="4" class="d-flex align-center auth-bg pa-10 pb-0">
          <v-row>
            <v-col cols="12" sm="8" md="6" lg="12" class="mx-auto">
              <v-card flat>
                <v-card-text>
                  <p class="text-2xl font-weight-semibold text--primary mb-2">이자경수학 학원에 오신걸 환영합니다! 👋🏻</p>
                  <p class="mb-2">오늘도 즐거운 하루 되시길 바랍니다.</p>
                </v-card-text>
                ...
                <v-card-text>
                  <v-alert text color="primary">
                    ...
                    <small> Client Email: <strong>client@materio.com</strong> / Pass: <strong>client</strong> </small>
                  </v-alert>
                </v-card-text>
                ...
                <v-card-text>
                  <v-form ref="loginForm" @submit.prevent="handleFormSubmit">
                    ...
                    <v-btn block color="primary" type="submit" class="mt-6" :loading="triggerLoading"> 로그인 </v-btn>
                  </v-form>
                </v-card-text>
...
export default {
  setup() {
     ...
     /*
      axios
        ...
        .then(response => {
          ...
          console.log('first step',accessToken)
          return response
        })
        .then(() => {
          axios.get('/auth/me').then(response => {
            ...
            console.log('second step2',user.ability)
            console.log('thirth step1',userAbility)
   ...
   axios.post('/api/logout').then(res => {console.log('res',res)}).catch(err=>{console.log('err',err)})
   axios
        .post('/api/login', { username: email.value, password: password.value })
        .then(response => {
          const { accessToken } = response.data
        ...
        .then(() => {
          ...
          axios.get('/api/user').then(response => {
            const user = {
              ...
              role: response.data.role.level,
              avatar:"",
              ability: response.data.role.ability,
            }
            const { ability: userAbility } = user
            console.log('second step',response)
        ...
        .catch(error => {
          ...
          triggerLoading.value = false
          // errorMessages.value = error.response.data.error
          errorMessages.value = getErrorMsg(error.response.data.errors)
        })
    }
    ...
    const getErrorMsg = (errors) =>{
      ...
      return `아이디 혹은 비밀번호를 확인해주세요. (${errors.username})`
    }
