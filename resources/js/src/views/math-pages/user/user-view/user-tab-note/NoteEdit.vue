...
<template>
  <v-dialog v-model="isNoteEditDialogVisible" persistent max-width="600px">
    ...
    <v-card>
      ...
      <v-card-text>
        <v-container>
          <v-form ref="form" v-model="valid" @submit.prevent="onSubmit">
            <v-row>
              <v-col cols="12">
                <v-text-field label="제목" v-model="noteDataLocal.title" :rules="[validators.required]">
                </v-text-field>
              </v-col>
              <v-col cols="12">
                <v-select label="종류" v-model="noteDataLocal.type" :items="['신규', '이탈', '정기상담']"
                  :rules="[validators.required]" dense></v-select>
              </v-col>
              <v-col cols="12">
                <v-textarea filled label="내용" v-model="noteDataLocal.text" :rules="[validators.required]" auto-grow
                  value=""></v-textarea>
              </v-col>
            </v-row>
          </v-form>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="error" @click="$emit('update:is-note-edit-dialog-visible', false)">
          닫기
        </v-btn>
